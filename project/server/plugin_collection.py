...
class Plugin(object):
    ...
    def load_input(self,test_dict = {'test':'test1','best':'best1'}):
...
class PluginCollection(object):
    ...
    def filter_plugin(self, argument):
        """Filters pulugin based on description
        """
        print()
        cls = []
        print(f'finding plugin with argument {argument}:')
        for plugin in self.plugins:
            print(f'Matching {argument} in  {plugin.description} {argument in plugin.description}')
            if argument in plugin.description:
                cls.append(plugin)
        return cls
            ...
            if not ispkg:
                ...
                for (_, c) in clsmembers:
                    ...
                    if issubclass(c, Plugin) & (c is not Plugin):
                        ...
                        input_data = {'a':'5'}
                        self.plugins.append(c(input_data))
